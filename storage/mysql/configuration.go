...
func (s *Storage) loadConfiguration() {
	...
	if s.server, found = os.LookupEnv(util.Server); !found {
		log.Fatal().Msg("The " + util.Server + " variable has not been set")
		os.Exit(1)
	}

	if s.database, found = os.LookupEnv(util.Database); !found {
		log.Fatal().Msg("The " + util.Database + " variable has not been set")
		os.Exit(1)
	}
	...
	if s.password, found = os.LookupEnv(util.Password); !found {
		log.Fatal().Msg("The " + util.Password + " variable has not been set")
		os.Exit(1)
	}
